...
func (s *AutoService) AutoRespond(token, resumeID string, vacancies []hhclient.Vacancy) error {
	...
	excluded := []string{"администратор", "курьер", "кассир", "автор", "менеджер", "руководитель", "бариста", "архитектор", "дизайнер",
		"бровист", "визажист"}

	// Список компаний, на которые не нужно откликаться
	excludedCompanies := []string{"ozon", "wildberries", "т-банк", "мтс", "магнит", "суточно.ру", "VK", "Вконтакте", "МойОфис", "Солюшен", "Группа Компаний Астрал"}
